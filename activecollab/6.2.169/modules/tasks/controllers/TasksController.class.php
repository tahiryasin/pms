...
class TasksController extends ProjectController
{
    ...
    public function for_screen(Request $request, User $user)
    {
        ...
        $collection_name = 'for_screen_'. $this->active_project->getId() . '_status_';

        if (!empty($params) && array_key_exists('status', $params)) {
            $collection_name .= (string) $params['status'];
        } else {
            $collection_name .= 'all';
        }
    ...
    public function edit(Request $request, User $user)
    {
        if ($this->active_task->isLoaded() && $this->active_task->canEdit($user)) {
            ...
            if (!$this->active_project->getMembersCanChangeBillable() && $user->isMember(true) && array_key_exists('is_billable', $attributes)) {
