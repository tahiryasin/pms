...
use ActiveCollab\Module\System\Events\DataObjectLifeCycleEvents\ProjectEvents\ProjectRestoredFromTrashEvent;
use Angie\Inflector;
...
class Project extends BaseProject implements RoutingContextInterface, IConfigContext, IIncomingMail
{
    ...
    const BUDGET_TYPE_PAY_AS_YOU_GO = 'pay_as_you_go';
    const BUDGET_NOT_BILLABLE = 'not_billable';
    ...
    public function removeMembers($users, $additional = null)
    {
        ...
        try {
            ...
            DataObjectPool::announce(new ProjectMembershipRevokedEvent($this));
    ...
    public function restoreFromTrash($bulk = false)
    {
        try {
            ...
            DataObjectPool::announce(new ProjectRestoredFromTrashEvent($this));
...
WHERE billable_status != 0 AND is_trashed != 1 AND internal_rate = 0 AND
      (
        ...
        $time_records_without_internal_rate = DB::executeFirstCell($query, $this->getId(), $this->getId()) ?? 0;

        return $time_records_without_internal_rate ? true : false;
    }
}
