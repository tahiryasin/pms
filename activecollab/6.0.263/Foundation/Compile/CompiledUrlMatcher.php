...
class CompiledUrlMatcher extends BaseCompiledUrlMatcher
{
    protected function matchRouteFrom(string $path, string $query_string): ?MatchedRouteInterface
    {
        ...
        if (preg_match('/^stopwatches\\/resume\\/(\\d+)$/', $path, $matches)) {
        ...
        } elseif (preg_match('/^stopwatches\\/pause\\/(\\d+)$/', $path, $matches)) {
            $url_params = $this->valuesFromMatchedPath(
                [
                        'id',
                    ],
        ...
        } elseif (preg_match('/^stopwatches\\/start$/', $path, $matches)) {
            $url_params = $this->valuesFromMatchedPath(
                [],
                [
                        'controller' => 'stopwatch',
        ...
        } elseif (preg_match('/^projects\\/([a-z0-9\\-\\._]+)\\/tasks\\/filters$/', $path, $matches)) {
            $url_params = $this->valuesFromMatchedPath(
                [
                        'project_id',
                    ],
