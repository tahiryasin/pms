...
class TaskLists extends BaseTaskLists
{
    ...
    public static function findSuccessiveByTaskLists(TaskList $task_list)
    {
        ...
        if ($start_on instanceof DateValue) {
            return self::find([
                'conditions' => ['project_id = ? AND start_on > ? AND is_trashed = ? AND id != ?', $task_list->getProjectId(), $start_on, false, $task_list->getId()],
                'order' => 'start_on',
            ]);
        }
    ...
    public static function getIdNameMap($filter = null)
    {
        if ($filter instanceof Project) {
            $rows = DB::execute('SELECT id, name FROM task_lists WHERE project_id = ? AND is_trashed >= ? ORDER BY ' . self::getDefaultOrderBy(), $filter->getId(), false);
        } elseif (is_array($filter)) {
            $rows = DB::execute('SELECT id, name FROM task_lists WHERE id IN (?) AND is_trashed = ? ORDER BY ' . self::getDefaultOrderBy(), $filter, false);
        } else {
            $rows = DB::execute('SELECT id, name FROM task_lists WHERE is_trashed = ? ORDER BY ' . self::getDefaultOrderBy(), false);
        }
