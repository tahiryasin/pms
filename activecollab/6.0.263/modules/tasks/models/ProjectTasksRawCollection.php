...
class ProjectTasksRawCollection extends CompositeCollection
{
    ...
    private function getTaskIds(): ?array
    {
        if ($this->task_ids === false) {
            ...
            if ($this->task_status_filter === 'open') {
    ...
    private function getTasksCollection(): ?DBResult
    {
        if ($this->tasks_collection === false) {
            $this->tasks_collection = DB::execute(
                'SELECT t.id, t.name, t.assignee_id, t.start_on, t.due_on, t.completed_on, t.position, t.task_list_id, t.project_id, t.is_hidden_from_clients, t.is_important, t.is_trashed, t.task_number, t.created_from_recurring_task_id
                  FROM tasks t
    ...
    public function execute()
    {
        ...
        foreach ($tasks as $task) {
            $result[] = [
                ...
                'completed_on' => $task['completed_on'] ? new DateValue($task['completed_on']) : null,
                ...
                'open_subtasks' => Subtasks::countByTaskId((int) $task['id'], $task['completed_on'] !== null)[0],
