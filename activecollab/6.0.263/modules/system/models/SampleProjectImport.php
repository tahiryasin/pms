...
final class SampleProjectImport implements ProjectImportInterface
{
    ...
    {
        $current_job_type_names = DB::executeFirstColumn('SELECT name FROM job_types');

        if (count($job_types) > 0) {
            foreach ($job_types as $job_type) {
                if (!in_array($job_type['name'], $current_job_type_names)) {
                    JobTypes::create([
                        'name' => $job_type['name'],
    ...
    {
        $task_dependencies = is_array($template['task_dependencies']) ? $template['task_dependencies'] : [];
