...
use ActiveCollab\ActiveCollabJobs\Jobs\Shepherd\SyncInstanceOwners;
use ActiveCollab\ActiveCollabJobs\Jobs\Shepherd\UpdateInstanceStats;
...
class Users extends BaseUsers
{
    ...
    private static function prepareUsersCollection($collection_name, $user)
    {
        ...
        $collection->setPreExecuteCallback(function ($ids) {
            ...
            self::preloadDailyCapacityFor($ids);
        });
    ...
    public static function changeUserType(User $user, $new_class, $custom_permissions, User $by)
    {
        if (self::isAvailableUserClass($new_class)) {
            ...
            $should_sync_owners = AngieApplication::isOnDemand() && OnDemand::shouldSyncOwnersOnUserTypeChange($user, $new_class);
            ...
            try {
                ...
                if (AngieApplication::isOnDemand()) {
                    if (!AngieApplication::isOnDemandNextGen()) {
                        AngieApplication::jobs()->dispatch(
                            new UpdateInstanceStats(
                                [
                    ...
                    // Send a request to resync owners in Cloud, in case that user is upgraded or downgraded from Owner role
                    if (!empty($should_sync_owners)) {
                        if (AngieApplication::isOnDemandNextGen()) {
                            AngieApplication::shepherdSyncer()->syncAccountOwner($user);
                        } else {
                            AngieApplication::jobs()->dispatch(
                                new SyncInstanceOwners(
                                    [
                                        'instance_id' => AngieApplication::getAccountId(),
                                        'delay' => 1,
                                    ]
    ...
    /**
     * Return number of chargable users in the system.
     *
     * @return int
     */
    public static function countChargableUsers()
    {
        $count_chargable_users = 0;
        ...
        foreach ($users as $user) {
            if ($user->isChargeable()) {
                $count_chargable_users++;
            }
        }

        return $count_chargable_users;
    }

    /**
     * Return array of Chargable users.
     *
     * @return array
     */
    public static function getChargableUsers()
    {
        $chargeable = [];

        /** @var User $user */
        foreach (self::getActiveUsers() as $user) {
            if ($user->isChargeable()) {
                $chargeable[] = $user;
            }
        }
            ...
            try {
                ...
                if ($uploaded_file) {
                    if ($current_avatar_location) {
                        AngieApplication::storage()->deleteFileByLocationAndType(
    ...
    private static $user_daily_capacity = false;

    public static function preloadDailyCapacityFor(array $user_ids)
    {
        if (self::$user_daily_capacity === false) {
            $users_with_custom_values = [];
            self::$user_daily_capacity = array_fill_keys($user_ids, []);

            if ($rows = DB::execute('SELECT parent_id, value FROM config_option_values WHERE name = ? AND parent_type = ? AND parent_id IN (?)', 'user_daily_capacity', User::class, $user_ids)) {
                foreach ($rows as $row) {
                    self::$user_daily_capacity[$row['parent_id']] = unserialize($row['value']);
                    $users_with_custom_values[] = $row['parent_id'];
                }
            }

            if ($diff_ids = array_diff($user_ids, $users_with_custom_values)) {
                $default_daily_capacity = ConfigOptions::getValue('user_daily_capacity');

                foreach ($diff_ids as $id) {
                    self::$user_daily_capacity[$id] = $default_daily_capacity;
                }
            }
        }
    }

    public static function getDailyCapacityFor(User $user)
    {
        if (isset(self::$user_daily_capacity[$user->getId()])) {
            return self::$user_daily_capacity[$user->getId()];
        } else {
            return ConfigOptions::getValueFor('user_daily_capacity', $user);
        }
    }
    ...
    public static function resetState()
    {
        ...
        self::$user_daily_capacity = false;
    }
