...
class StorageUsageSnapshot implements StorageUsageSnapshotInterface
{
    ...
    )
    {
        ...
        foreach ($usage_data_by_service as $service => $usage_data) {
            if (!in_array($service, StorageInterface::SERVICES)) {
    ...
    {
        return $this->getUsageByService(StorageInterface::SERVICE_LOCAL) +
            $this->getUsageByService(StorageInterface::SERVICE_WAREHOUSE);
    }
