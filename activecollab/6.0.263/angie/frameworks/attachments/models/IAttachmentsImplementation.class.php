...
trait IAttachmentsImplementation
{
    ...
    public function IAttachmentsImplementation()
    {
        ...
        $this->registerEventHandler('on_after_save', function ($is_new) {
            $changes = 0;
    ...
    {
        return Attachments::find(['conditions' => Attachments::parentToCondition($this)]);
    }
    ...
    {
        return Attachments::find(['conditions' => DB::prepare(Attachments::parentToCondition($this) . ' AND disposition = ?', IAttachments::INLINE)]);
    }
    ...
    public function cloneAttachmentsTo(IAttachments $to)
    {
        ...
        if ($rows = DB::execute('SELECT id, type, name, mime_type, size, location, md5, created_on, created_by_id, created_by_name, created_by_email, raw_additional_properties, is_hidden_from_clients FROM attachments WHERE parent_type = ? AND parent_id = ?', $from_class, $from_id)) {
            try {
                ...
                foreach ($rows as $row) {
                    if ($row['type'] === WarehouseAttachment::class) {
                        $this->attachWarehouseFile(Attachments::findById($row['id']), null, $to);
                    } elseif ($row['type'] === GoogleDriveAttachment::class || $row['type'] === DropboxAttachment::class) {
                        $this->attachExternalFile(Attachments::findById($row['id']), null, $to);
                    } else {
                        ...
                        if (is_file($source_path)) {
                            ...
                            $to_insert[] = DB::prepare("($project_id, $to_class, $to_id, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", $row['type'], $row['name'], $row['mime_type'], $row['size'], $new_location, $row['md5'], $row['created_on'], $row['created_by_id'], $row['created_by_name'], $row['created_by_email'], $row['raw_additional_properties'], $row['is_hidden_from_clients']);
                        }
                    }
                }
