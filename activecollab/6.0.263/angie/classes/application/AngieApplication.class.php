...
use ActiveCollab\Module\OnDemand\Utils\ChargableUsersResolver\ChargableUsersResolver;
use ActiveCollab\Module\OnDemand\Utils\ChargableUsersResolver\ChargableUsersResolverInterface;
...
use ActiveCollab\Module\OnDemand\Utils\MrrCalculator\MrrCalculator;
...
use Angie\Storage\Capacity\CapacityCalculator;
use Angie\Storage\Capacity\CapacityCalculatorInterface;
use Angie\Storage\Storage;
...
use Angie\Utils\OnDemandStatusResolver\OnDemandStatusResolver;
use Angie\Utils\OnDemandStatusResolver\OnDemandStatusResolverInterface;
...
final class AngieApplication
{
    ...
    public static function bootstrapForTest(string $pricing_model = 'plans_2013'): void
    {
        ...
        self::setOnDemandStatusResolver(
            new OnDemandStatusResolver(true)
        );
        ...
        if ($pricing_model !== 'plans_2013') {
            throw new LogicException(
                'Please update AngieApplication::bootstrapForTest() to support your pricing model.'
            );
        }
        ...
        if (empty(self::$container)) {
            if (empty(self::$frameworks_initialized) || empty(self::$modules_initialized)) {
                throw new RuntimeException(
                    'Container can be constructed only after frameworks and modules are initialied.'
                );
            }
    ...
    public static function &authentication()
    {
        if (empty(self::$authentication)) {
            $authorization_locator = new AuthorizationIntegrationLocator(
                self::isOnDemand(),
                self::isInDevelopment() || self::isInTestMode(),
                self::useShepherdAsIdp(),
    ...
    public static function &storage(): StorageInterface
    {
        if (!self::$storage) {
            self::$storage = new Storage(
                ...
                self::storageCapacityCalculator(),
     ...
     */
    public static function unsetStorageDelegate(): void
    {
        self::$storage = null;
    }
    ...
    public static function storageCapacityCalculator(bool $is_on_demand = null): CapacityCalculatorInterface
    {
        if (empty(self::$storage_capacity_calculator)) {
            if ($is_on_demand ?? self::isOnDemand()) {
                self::$storage_capacity_calculator = new CapacityCalculator(
    ...
    public static function currentUsage(): CurrentUsageInterface
    {
        ...
        if (self::shepherdAccountConfig()->getIsInstalled(self::getAccountId())) {
            $active_users = self::accountConfigReader()->getPricingModel() === PricingModelInterface::PRICING_MODEL_PER_SEAT_2018
                ? Users::countChargableUsers() : Users::countActiveUsers();

            $storage_used = self::getStorageUsage(true);
        }

        //@ToDo create current usage resolver depending of pricing model
        ...
        if ($is_on_demand && !self::$account_settings) {
            ...
            self::$account_settings = new AccountSettings(
                new AccountStatus(
                    ...
                    self::accountConfigReader()->isNonProfit()
                ),
                $plan,
                $pricing_model,
                self::currentUsage(),
    ...
    /**
     * @param  DateValue|null                     $current_date
     * @return FailedPaymentDaysResolverInterface
     */
    ...
    /**
     * @var ChargableUsersResolverInterface
     */
    private static $chargable_user_resolver;

    public static function &chargableUserResolver($is_on_demand = null): ChargableUsersResolverInterface
    {
        if ($is_on_demand === null) {
            $is_on_demand = self::isOnDemand();
        }

        if ($is_on_demand && !self::$chargable_user_resolver) {
            self::$chargable_user_resolver = new ChargableUsersResolver(
                Users::getChargableUsers(),
                [],
                self::accountConfigReader()->getPricingModel()
            );
        }

        return self::$chargable_user_resolver;
    }
    ...
    public static function &occupyAccountService($is_on_demand = null): OccupyAccountService
    {
        ...
        if ($is_on_demand && !self::$account_occupier) {
            self::$account_occupier = new OccupyAccountService(
                ...
                function ()
                {
                    /** @var WelcomeMailNotification $notification */
                    $notification = self::notifications()
                        ->notifyAbout('welcome_mail');
                    ...
                    $notification
                        ...
                        ->sendToUsers(Users::findFirstOwner());
                },
                self::eventsDispatcher()
            );
        }
    ...
    /**
     * @return ShepherdAccountConfigInterface|null
     */
    ...
    /**
     * @return AccountConfigReaderInterface
     */
     ...
     * Set tax resolver.
     *
     * @param TaxResolverInterface $tax_resolver
     */
     ...
     */
    private static $discount_resolver;

    public static function usetDiscountResolver()
    {
        self::$discount_resolver = null;
    }

    public static function &discountResolver()
    {
        if (!self::isOnDemand()) {
            throw new LogicException('Discount Resolver is available only for OnDemand');
        }

        if (!self::$discount_resolver) {
            self::$discount_resolver = new DiscountResolver(self::accountConfigReader()->isNonProfit());
        }

        return self::$discount_resolver;
    }
    ...
    public static function &chargableUserAddBalance()
    {
        ...
        if (!self::$chargable_user_add_balance) {
            self::$chargable_user_add_balance = new ChargableUserAddBalance(
                self::accountBalance(),
                self::balanceCalculator(),
                self::accountConfigReader()->getReferenceBillingDate(),
                self::accountConfigReader()->getNextBillingDate(),
    ...
    private static function getStripeKey(string $public_or_secret): string
    {
        ...
        $key_name = strtoupper($public_or_secret) . '_STRIPE_API_KEY';
    ...
    private static $on_demand_status_resolver;

    public static function onDemandStatusResolver(): OnDemandStatusResolverInterface
    {
        if (self::$on_demand_status_resolver === null) {
            self::$on_demand_status_resolver = new OnDemandStatusResolver(
                defined('IS_ON_DEMAND') && IS_ON_DEMAND
            );
        }

        return self::$on_demand_status_resolver;
    }

    public static function setOnDemandStatusResolver(?OnDemandStatusResolverInterface $on_demand_status_resolver)
    {
        self::$on_demand_status_resolver = $on_demand_status_resolver;
    }

    public static function isOnDemand()
    {
        return self::onDemandStatusResolver()->isOnDemand();
    }

    public static function isOnDemandNextGen(): bool
    {
        return self::isOnDemand();
    }

    public static function useShepherdAsIdp(): bool
    {
        return self::isOnDemand() && defined('USE_SHEPHERD_AS_IDP') && USE_SHEPHERD_AS_IDP;
    }
            ...
            foreach (self::getFrameworkNames() as $framework_name) {
                ...
                if ($framework instanceof AngieFramework) {
                    ...
                    throw new ClassNotImplementedError(
                        $framework_class,
                        $framework_class_path,
                        'Framwork definition class not found'
                    );
                }
            }
        }
    ...
    private static $update_non_profit;

    public static function updateNonProfitService($is_on_demand = null)
    {
        if ($is_on_demand === null) {
            $is_on_demand = self::isOnDemand();
        }

        if ($is_on_demand && !self::$update_non_profit) {
            return new UpdateNonProfitService(
                self::getAccountId(),
                self::accountSettings()->getAccountStatus(),
                self::initialSettingsCacheInvalidator(),
                self::log(),
    ...
    public static function featureFactory($is_on_demand = null)
    {
        if ($is_on_demand === null) {
            $is_on_demand = self::isOnDemand();
        }

        if ($is_on_demand && !self::$feature_factory) {
