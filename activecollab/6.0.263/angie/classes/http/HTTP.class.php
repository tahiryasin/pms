 ...
 * @see        http://pear.php.net/package/HTTP
 */
...
class HTTP
{
    ...
    public function head($url, $timeout = 10)
    {
        ...
        if (!isset($p['scheme'])) {
            ...
            return self::raiseError('Unsupported protocol: ' . $p['scheme']);
        }

        $port = isset($p['port']) ? $p['port'] : 80;

        if (!$fp = @fsockopen($p['host'], $port, $eno, $estr, $timeout)) {
            return self::raiseError("Connection error: $estr ($eno)");
        }
     ...
     * @param  int    $port     A new port number
     */
