 ...
 * @see        http://pear.php.net/package/HTTP_Header
 */
...
class HTTP_Header extends HTTP
{
     ...
     *                        sent
     */
    public function sendHeaders($keys = [], $include = true)
    {
        if (headers_sent()) {
            return false;
        }
     ...
     */
    public function redirect($url = null, $param = [], $session = false)
    {
        if (!isset($url)) {
    ...
    public function isInformational($http_code)
    {
        if ($status_type = $this->getStatusType($http_code)) {
            ...
            == HTTP_HEADER_STATUS_INFORMATIONAL;
        } else {
            return false;
        }
    }
    ...
    public function isClientError($http_code)
    {
        if ($status_type = $this->getStatusType($http_code)) {
            return $status_type[0]
            == HTTP_HEADER_STATUS_CLIENT_ERROR;
        } else {
            return false;
        }
    }
    ...
    public function isServerError($http_code)
    {
        if ($status_type = $this->getStatusType($http_code)) {
            return $status_type[0]
            == HTTP_HEADER_STATUS_SERVER_ERROR;
        } else {
            return false;
        }
    }
